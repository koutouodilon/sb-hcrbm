...
public class TargetBasedCRBM {
    ...
    private static void load_distinction_data(String data_folder){
        try {
            ...
            X_train_isMissing = DoubleMatrix.loadCSVFile(data_folder + "df_isMissing_target_rbm.csv");
    ...
    /**
     * Method to evaluate the T-CRBM with distinction
     * @param crbm
    ...
    private static void evaluate_distinction(DrugTargetCRBM crbm, String results_folder){
        ...
        for(int k = 0; k < X_train_direct.getRows(); k++){
            boolean[] missing = X_train_isMissing.getRow(k).toBooleanArray();

            double[][] input = { X_train_direct.getRow(k).toArray(), X_train_indirect.getRow(k).toArray() };
            double[][] prob = { probTarget_direct.getRow(k).toArray(), probTarget_indirect.getRow(k).toArray() };
    ...
    /**
     * Method to evaluate the T-CRBM without distinction
     * @param crbm
     * @param results_folder
     */
    private static void evaluate_no_distinction(DrugTargetCRBM crbm, String results_folder){
        ...
        for(int k = 0; k < X_train_direct.getRows(); k++){
            ...
            double[][] input_direct = { X_train_direct.getRow(k).toArray() };
            double[][] prob_direct = { probTarget_direct.getRow(k).toArray() };
            double[][] output_direct = DrugTargetCRBMTool.predict(crbm, input_direct, prob_direct, missing);

            double[][] input_indirect = { X_train_indirect.getRow(k).toArray() };
            double[][] prob_indirect = { probTarget_indirect.getRow(k).toArray() };
