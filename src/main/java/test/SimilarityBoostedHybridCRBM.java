...
public class SimilarityBoostedHybridCRBM {
    public static double[][][] X_test_drug_rbm;

    public static DoubleMatrix X_test_drug_rbm_direct;
    public static DoubleMatrix X_test_drug_rbm_indirect;
    public static DoubleMatrix X_test_drug_rbm_isMissing;
    ...
    public static double[][][] X_test_target_rbm;

    public static DoubleMatrix X_test_target_rbm_direct;
    public static DoubleMatrix X_test_target_rbm_indirect;
    public static DoubleMatrix X_test_target_rbm_isMissing;

    public static double[][][] probTargetTesting;
    public static DoubleMatrix probTargetTesting_direct;
    public static DoubleMatrix probTargetTesting_indirect;
        ...
        for(int split = 1; split <= n_splits; split++) {
            ...
            if(method == 1){
                ...
                for(int p = 0; p < numTargets; p++){
                    double[][] input_target = { X_test_target_rbm_direct.getRow(p).toArray(),  X_test_target_rbm_indirect.getRow(p).toArray()};
                    boolean[] missing = X_test_target_rbm_isMissing.getRow(p).toBooleanArray();
                    double[][] probTarget = { probTargetTesting_direct.getRow(p).toArray(),  probTargetTesting_indirect.getRow(p).toArray()};
                ...
                for(int d = 0; d < numDrugs; d++){
                    double[][] input_drug = { X_test_drug_rbm_direct.getRow(d).toArray(),  X_test_drug_rbm_indirect.getRow(d).toArray()};
                    boolean[] missing = X_test_drug_rbm_isMissing.getRow(d).toBooleanArray();
                    double[][] probDrug = { probDrugTesting_direct.getRow(d).toArray(),  probDrugTesting_indirect.getRow(d).toArray()};
                ...
                for(int p = 0; p < numTargets; p++){
                    boolean[] missing = X_test_target_rbm_isMissing.getRow(p).toBooleanArray();
                    double[][] input_target_direct = { X_test_target_rbm_direct.getRow(p).toArray() };
                    double[][] input_target_indirect = { X_test_target_rbm_indirect.getRow(p).toArray()};

                    double[][] probTarget_0 = { probTargetTesting_direct.getRow(p).toArray() };
                    double[][] probTarget_1 = { probTargetTesting_indirect.getRow(p).toArray()};
                ...
                for(int d = 0; d < numDrugs; d++){
                    boolean[] missing = X_test_drug_rbm_isMissing.getRow(d).toBooleanArray();
                    double[][] input_drug_direct = { X_test_drug_rbm_direct.getRow(d).toArray() };
                    double[][] input_drug_indirect = { X_test_drug_rbm_indirect.getRow(d).toArray()};

                    double[][] probDrug_0 = { probDrugTesting_direct.getRow(d).toArray() };
                    double[][] probDrug_1 = { probDrugTesting_indirect.getRow(d).toArray()};
    ...
    private static void load_distinction_data_target_rbm(String data_folder){
        try {
            X_test_target_rbm_direct = DoubleMatrix.loadCSVFile(data_folder + "0_df_X_test_target_rbm.csv");
            X_test_target_rbm_indirect = DoubleMatrix.loadCSVFile(data_folder + "1_df_X_test_target_rbm.csv");
            X_test_target_rbm_isMissing = DoubleMatrix.loadCSVFile(data_folder + "df_isMissing_target_rbm_testing.csv");

            probTargetTesting_direct = DoubleMatrix.loadCSVFile(data_folder + "0_df_probTargetTesting.csv");
            probTargetTesting_indirect = DoubleMatrix.loadCSVFile(data_folder + "1_df_probTargetTesting.csv");

            X_test_target_rbm = new double[2][][];
            X_test_target_rbm[0] = X_test_target_rbm_direct.toArray2();
            X_test_target_rbm[1] = X_test_target_rbm_indirect.toArray2();

            probTargetTesting = new double[2][][];
            probTargetTesting[0] = probTargetTesting_direct.toArray2();
            probTargetTesting[1] = probTargetTesting_indirect.toArray2();
    ...
    private static void evaluate_distinction(int numTargets, int numDrugs, double [][][] formated_results, String results_folder){
        ...
        for(int p = 0; p < numTargets; p++){
            boolean[] missing = X_test_target_rbm_isMissing.getRow(p).toBooleanArray();
            double[][] input = { X_test_target_rbm_direct.getRow(p).toArray(),  X_test_target_rbm_indirect.getRow(p).toArray()};
    ...
    private static void load_distinction_data_drug_rbm(String data_folder){
        try {
            X_test_drug_rbm_direct = DoubleMatrix.loadCSVFile(data_folder + "0_df_X_test_drug_rbm.csv");
            X_test_drug_rbm_indirect = DoubleMatrix.loadCSVFile(data_folder + "1_df_X_test_drug_rbm.csv");
            X_test_drug_rbm_isMissing = DoubleMatrix.loadCSVFile(data_folder + "df_isMissing_drug_rbm_testing.csv");
            ...
            X_test_drug_rbm = new double[2][][];
            X_test_drug_rbm[0] = X_test_drug_rbm_direct.toArray2();
            X_test_drug_rbm[1] = X_test_drug_rbm_indirect.toArray2();
