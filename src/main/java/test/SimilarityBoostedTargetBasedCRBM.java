...
/**
 * Similarity Boosted Target-based CRBM
 */
public class SimilarityBoostedTargetBasedCRBM {
    ...
    public static void main(String[] args) {
        ...
        for(int split = 1; split <= n_splits; split++) {
            ...
            DrugTargetCRBM t_crbm;

            if(backup_option == 'y'){
                t_crbm = DrugTargetCRBMTool.load("./backup/" + backup_folder_split, numDrugs, hidUnits, lr, weightCost, momentum, numClass);
                File dir = new File(results_folder_split);
                dir.mkdirs();
            }else{
                // We train the model
                t_crbm = DrugTargetCRBMTool.fit(X_train, probTarget, isMissing, hidUnits, epochs, lr, momentum, weightCost, monitoring_folder_split);

                // We save the trained model
                t_crbm.save(backup_folder_split);
            }

            // We generate the evaluation file
            if(method == 1){
                evaluate_distinction(t_crbm, results_folder_split);
            }else{
                evaluate_no_distinction(t_crbm, results_folder_split);
            }
        }
    ...
    private static void load_distinction_data(String data_folder){
        try {
            X_train_direct = DoubleMatrix.loadCSVFile(data_folder + "0_df_X_train_target_rbm.csv");
            X_train_indirect = DoubleMatrix.loadCSVFile(data_folder + "1_df_X_train_target_rbm.csv");
            X_train_isMissing = DoubleMatrix.loadCSVFile(data_folder + "df_isMissing_target_rbm_training.csv");

            probTarget_direct = DoubleMatrix.loadCSVFile(data_folder + "0_df_probTargetTraining.csv");
            probTarget_indirect = DoubleMatrix.loadCSVFile(data_folder + "1_df_probTargetTraining.csv");
            ...
            probTarget = new double[2][][];
            probTarget[0] = probTarget_direct.toArray2();
            probTarget[1] = probTarget_indirect.toArray2();


            X_test_direct = DoubleMatrix.loadCSVFile(data_folder + "0_df_X_test_target_rbm.csv");
            X_test_indirect = DoubleMatrix.loadCSVFile(data_folder + "1_df_X_test_target_rbm.csv");
            X_test_isMissing = DoubleMatrix.loadCSVFile(data_folder + "df_isMissing_target_rbm_testing.csv");
    ...
    private static void load_no_distinction_data(String data_folder) {
        try {
            ...
            probTarget = new double[1][][];
            probTarget[0] = probTarget_direct.add(probTarget_indirect).toArray2();
    ...
    private static void evaluate_distinction(DrugTargetCRBM crbm, String results_folder){
        ...
        for(int k = 0; k < X_test_direct.getRows(); k++){
            ...
            double[][] prob = { probTargetTesting_direct.getRow(k).toArray(), probTargetTesting_indirect.getRow(k).toArray() };
    ...
    private static void evaluate_no_distinction(DrugTargetCRBM crbm, String results_folder){
        ...
        for(int k = 0; k < X_test_direct.getRows(); k++){
            ...
            double[][] prob_direct = { probTargetTesting_direct.getRow(k).toArray() };
            ...
            double[][] prob_indirect = { probTargetTesting_indirect.getRow(k).toArray() };
